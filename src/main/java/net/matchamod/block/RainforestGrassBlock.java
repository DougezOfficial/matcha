...
public class RainforestGrassBlock extends GrassBlock {
    public RainforestGrassBlock(Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!world.isClient) {
            ...
            if (!canSurvive(state, world, pos)) {
                world.setBlockState(pos, ModBlocks.RAINFOREST_SOIL.getDefaultState());
            } else {
                if (world.getLightLevel(pos.up()) >= 9) {
                    for (int i = 0; i < 4; ++i) {
                        ...
                        if (targetState.isOf(ModBlocks.RAINFOREST_SOIL)
